...
Param(
    ...
    [string] $ResourceGroupName = 'SonarQubeARM',
...
if ($UploadArtifacts) {
    ...
    if (Test-Path $DSCSourceFolder) {
        ...
        $ArchiveFile = Join-Path $ArtifactStagingDirectory $dscZipFile
        Remove-Item -Path $ArchiveFile -ErrorAction SilentlyContinue
        [System.IO.Compression.ZipFile]::CreateFromDirectory($DSCSourceFolder, $ArchiveFile)
    }

	# Delete all the files in the $ArtifactsLocation except the DSC .zip file so all that cruft doesn't get copied to the storage acct
	Write-Verbose "Preparing to clean out '$DSCSourceFolder'"
	if(Test-Path $DSCSourceFolder){
		Write-Verbose "    '$DSCSourceFolder' was found...deleting contents"
		Get-ChildItem -Path  $DSCSourceFolder -Recurse -exclude $dscZipFile |
		Select -ExpandProperty FullName |
		sort length -Descending |
		Remove-Item -force 
	}else{
		Write-Verbose "    '$DSCSourceFolder' was not found"
	}
	Write-Verbose "Finished cleaning out '$DSCSourceFolder'"
